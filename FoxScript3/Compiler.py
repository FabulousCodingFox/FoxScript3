...
def copyFolderTree(source,target):
...
class Project:
    ...
    def compile(self):
        ...
        logging.info("[Project] Adding Dependencies")


        s=getFailSafe(self.config,"DEPENDENCIES",False)
        if(s!=False):
            localwasd=getFailSafe(self.config["DEPENDENCIES"],"LocalWASD",False)
            nexus=getFailSafe(self.config["DEPENDENCIES"],"Nexus",False)

            if localwasd:
                logging.info("[Project] Installing dependency LocalWASD by Miestrode")
                copyFolderTree(os.path.join(_dir_,"dependencies","local.wasd"),os.path.join(self.config["TARGET"]["path"]["datapack"],"data","local.wasd"))
                self.config["SCHEDULES"].append({"timing":"load","path":"local.wasd:base/install"})
                self.config["SCHEDULES"].append({"timing":"tick","path":"local.wasd:base/loop"})
...
Optional Dependencies that can be injectet into the target datapack:
